...
LDAPPreviewValuesSchema_BaseDn_description = "List of Base DNs, e.g. 'C=example,DC=com' or 'DC=hello' or 'OU=Blah'"
LDAPPreviewValuesSchema_SearchScope_description = "List containing one single string: ['0'] = Base; ['1'] = One Level; ['2'] = Sub tree;"
LDAPPreviewValuesSchema_IsGC_description = "True if this report searches global catalog, False otherwise"
...
class LDAPPreviewValuesSchema(BaseModel):
    ...
    """
    BaseDn: list[str]                   = Field(description=LDAPPreviewValuesSchema_BaseDn_description)  # TODO is there any governing logic independent of environment (which means we can further specify here)
    SearchScope: Literal["0", "1", "2"] = Field(description=LDAPPreviewValuesSchema_SearchScope_description)  # TODO Note: This value will have to be post-procssesed to be a single item in a list, i.e. ["0"] or ["1"] or ["2"]
    IsGC: bool                          = Field(description=LDAPPreviewValuesSchema_IsGC_description)  # TODO Note: This value will have to be post-processed to be a single item in a list, i.e. [True] or [False]
    TreeValues: Dict[str, str]          = Field(description=LDAPPreviewValuesSchema_TreeValues_description)  # TODO Note: This value will have to be post-processed to be a list of strings of the form "<field>;<value>", e.g. ["objectClass;sitesContainer", "objectCategory;blah"] or ["objectClass;user", "whenCreated;Abs:2024-01-01T00:00:00.0000000", "whenCreated;Rel:1W"]